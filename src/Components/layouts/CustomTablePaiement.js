...
export default function DataGridDemo() {
    ...
    const RenderCellComponent = (params) => {
        ...
        const deletePaiement = async () => {
            const id = service.id;
            try {
                const resp = await api.deletepaiement(id);
 ...
 */}
                ...
                >
                    ...
                    <div style={{ display: "flex", gap: "20px", marginLeft: "270px" }}>
                        <button onClick={deletePaiement} style={{ padding: "15px 20px", backgroundColor: "#b1e0e9", border: "none", color: "#fff" }}>Oui</button>
    ...
    const readallpaiement = async () => {
        try {
            const reponse = await api.readallpaiement()
